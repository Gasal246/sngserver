import { formatResponse, hasDuplicate, removeDuplicates } from "../../../helpers";
import { Request, Response } from "express";
import { getServiceListOfCampIds } from "../../../services/camp_assign_services";

export const getCampAssignedServices = async (req: Request | any, res: Response) => {
    try {
        ...
        if (campIds.length === 0) {
            const data = formatResponse(400, true, "You have selected invalid ids", null);
            res.status(400).json(data);
            return;
        }
        const { data: clientData } = req.decodedToken;
        const services = await getServiceListOfCampIds(campIds, status ? status.toString() : 1);
