...
import { attachServiceToClientFuncion, changeStatusOfService, isServiceAlreadyAttached } from "../../services/services";
...
export const attachServiceToClient = async (req: Request, res: Response) => {
    try {
        ...
        const data = formatResponse(200, false, "service successfully attached!", attached);
...
export const changeStatusOfAttachedService = async (req: Request, res: Response) => {
    try {
        const { id, status } = req.body;

        const updated = await changeStatusOfService(id, status);
        
        const data = formatResponse(200, false, "assigned service: status updated!", updated);
        res.status(200).json(data);
        return;
    } catch (e: any) {
        const data = formatResponse(500, true, e.message, null);
