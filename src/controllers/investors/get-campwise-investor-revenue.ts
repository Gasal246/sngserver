...
import {
  getInvestorAssignedCampsList,
  isInvestorAssignedToCamp,
} from "../../services/investor_assign_camps";
import {
  getRevenuePercentWithinDate,
  getRevenueScheduleInvestorCamp,
  investorRevenueSchedulesForServiceId,
} from "../../services/investor_revenue_schedule";
import { getInvestorServiceOfCamp } from "../../services/investor_assign_services";
import {
  getCampTransactionsWithinDates,
  getServiceBasedTransactionsOfCamp,
...
export const getInvestorCampWiseRevenue = async (
  req: Request | any,
  res: Response
) => {
  try {
    ...
    const getTransactions = await getTransactionsOfClientWithinDates(
      new Date(sd),
      new Date(ed || new Date()),
      investor.client_id!.toString()
    );

    const assignedCamps = await getInvestorAssignedCampsList(
      investorId,
      status || "active"
    );
    ...
    for (const camp of camps) {
      const transactions = getTransactions.filter((transaction: any) =>
...
export const getInvestorCampRevenue = async (
  req: Request | any,
  res: Response
) => {
  try {
    ...
    const assignedServices = await getInvestorServiceOfCamp(
      investorId,
      campId,
      status
    );
    const camp_transactions = await getCampTransactionsWithinDates(
      investor?.client_id!.toString(),
      campId,
      new Date(sd),
      new Date(ed)
    ...
    const services: any[] = assignedServices.map(
      (service) => service?.service_id
    );

    let obj = [];
    for (const service of services) {
      const transactions = camp_transactions.filter((transaction: any) =>
