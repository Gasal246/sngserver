...
export const getInvestorMembershipUsers = async (req: Request | any, res: Response) => {
    try {
        ...
        const isServiceAssignedToInvestor = await isInvestorServiceAlreadyAssigned(investorId, service._id.toString(), campId);
        if (!isServiceAssignedToInvestor) {
            const data = formatResponse(404, true, "Service not assigned to investor in this camp", null);
            res.status(404).json(data);
            return;
        }
        ...
        const activeOrdersByCampUsers = await getOrdersByUserIdsandCamp(campUsers.map((data: any) => createObjectId(data?.user_data?.id)), createObjectId(campId));
        const activePlanUsers = new Set<string>();
        activeOrdersByCampUsers.map((data: any) => activePlanUsers.add(data?.user_id?.toString()));

        const activeUsers = campUsers.filter((data: any) => activePlanUsers.has(data?.user_id?.toString()));
        const inactiveUsers = campUsers.filter((data: any) => !activePlanUsers.has(data?.user_id?.toString()));

        const data = formatResponse(200, false, "Camp users fetched", { activeUsers, inactiveUsers });
