...
import { getServiceIdTransactionWithinDates, getTransactionsOfClientWithinDates } from "../../services/user_transactions";
...
export const getInvestorServiceWiseRevenue = async (req: Request | any, res: Response) => {
    try {
        ...
        const investorAssignedServices = await getAllInvestorAssignedServices(investorId, status ? parseInt(status) : undefined);

        const serviceIds = investorAssignedServices.map((service: any) => service?.service_id?._id?.toString() || '') || [];

        const transactions = await getTransactionsOfClientWithinDates(new Date(sd), new Date(ed || new Date()), investor.client_id!.toString());

        const filteredTransactions = transactions.filter(transaction => serviceIds.includes(transaction?.serviceId?.toString() || ''));


        const services: any = investorAssignedServices.map(service => service?.service_id);

        const obj = [];

        for(const service of services) {
            const transactions = filteredTransactions.filter((transaction: any) => transaction?.serviceId?.equals(service?._id));
            let result: any[] = [];
        ...
        const data = formatResponse(200, false, "Servicewise revenue fetched successfully", {
            investor,
            services: obj
        });
...
export const getInvestorServiceRevenue = async (req: Request | any, res: Response) => {
    try {
        const { investorId, sd, ed, status }: any = req.query;
        ...
        const user_transactions = await getServiceIdTransactionWithinDates(investor.client_id!.toString(), serviceId, new Date(sd), new Date(ed || new Date()));

        let obj: any[] = [];

        for( const camp of camps ) {
            const transactions = user_transactions.filter((transaction: any) => transaction?.campId?.equals(camp?._id));
