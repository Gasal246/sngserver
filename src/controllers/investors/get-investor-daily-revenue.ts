...
export const getInvestorDailyRevenue = async (req: Request | any, res: Response) => {
    try {
        ...
        if(revenueSchedules?.length === 0) {
            const data = formatResponse(400, true, "No revenue schedules found.", null);
        ...
        const serviceIds = investorServices.map(service => service.service_id?.toString()) || [];

        const transactions = await getUserTransactionsWithStartAndEndDateIncludingServiceId(thirtyDaysAgo, today, serviceIds as string[]);
        ...
        for( const transaction of transactions) {
            ...
            const matchingSchedule = revenueSchedules.find((schedule: any) => {
                const matchesService = schedule.service_id.equals(transaction.serviceId);
                const matchesCamp = schedule.camp_id ? schedule.camp_id.equals(transaction.campId) : true;
                const withinDateRange =
                (!schedule.start_date || transaction.createdAt >= schedule.start_date) &&
            ...
            if(matchingSchedule) {
                ...
                const investorRevenue = ((matchingSchedule.revenue_percent as number) / 100) * (transaction.amount as number);

                // aggregating revenue by date
                if (dailyRevenueMap.has(transactionDate)) {
                    dailyRevenueMap.set(transactionDate, dailyRevenueMap.get(transactionDate) + investorRevenue);
                } else {
        ...
        const result = Array.from(dailyRevenueMap.entries()).map(([date, investor_revenue]) => ({
          date,
        ...
        const data = formatResponse(200, true, "Daily Service Data Fetched", result);
