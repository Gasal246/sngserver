...
export const getClientInvestorRevenueSchedule = async (req: Request, res: Response) => {
    try {
        ...
        const service_assigned = await isInvestorServiceAlreadyAssigned(investorId, serviceId, campId);
        if(!service_assigned) {
        ...
        const list = await getInvestorRevenueScheduleList(investorId, campId, serviceId);
        
        const data = formatResponse(200, false, "Revenue schedule fetched successfully", {
            list,
            investor_data,
            camp_data,
            service_data
