...
import { getServiceIdTransactionWithinDates, getTransactionsByCampIdsWithinDate } from "../../services/user_transactions";
import { getAccountantById } from "../../services/accountant";
import { getCampsByAttachedServiceId, getServicesByAttachedCampId } from "../../services/camp_assign_services";
...
export const getSalesData = async (req: Request | any, res: Response) => {
    try {
        ...
        const accountant = await getAccountantById(accountatId, req.decodedToken?.data?.client_id);
        if (!accountant) {
        ...
        const accountant_camps = await getCampByAccountant(createObjectId(accountatId), status ? status+'' : "1");
        ...
        const campIds = accountant_camps.map((camp) => createObjectId(camp.camp_id));

        const transactions = await getTransactionsByCampIdsWithinDate(campIds, new Date(start_date), new Date(end_date));

        let obj: any[] = [];

        for (const camp of accountant_camps) {
            const campTransactions = transactions.filter((transaction: any) => transaction?.campId?.equals(camp?.camp_id));
            const campsSalesTotal = campTransactions.reduce((sum: number, transaction: any) => sum + (transaction?.sales_amount || 0), 0);
            
            // service grouping and formating...
            const serviceCountMap: Record<string, { service_id: string, service_name: string, total_sales_amount: number }> = {};
            campTransactions.forEach((transaction: any) => {
                const serviceId = transaction?.serviceId?._id?.toString();
                if(serviceCountMap[serviceId]) {
                    serviceCountMap[serviceId].total_sales_amount += (transaction?.sales_amount || 0);
                } else {
        ...
        const data = formatResponse(200, false, "Sales data fetched successfully", obj);
...
export const getCampSalesData = async (req: Request | any, res: Response) => {
    try {
        ...
        const transactions = await getTransactionsByCampIdsWithinDate([createObjectId(campId)], new Date(start_date), new Date(end_date));
        
        const campAssignedServices = await getServicesByAttachedCampId(campId);

        const serviceCountMap: Record<string, { service_id: string, service_name: string, service_status: number, assign_status: number, total_sales_amount: number }> = {};
        ...
        const data = formatResponse(200, false, "Sales data fetched successfully", Object.values(serviceCountMap));
        res.status(200).json(data);
        return;
    } catch (error: any) {
        const data = formatResponse(500, true, error.message, null);
...
export const getServiceSalesData = async (req: Request | any, res: Response) => {
    try {
        ...
        const campCountMap: Record<string, { camp_id: string, camp_name: string, camp_city: string, camp_status: number, assign_status: number, total_sales_amount: number }> = {};
        ...
        transactions.forEach((transaction: any) => {
            ...
            if(campCountMap[campId]) {
                campCountMap[campId].total_sales_amount += (transaction?.sales_amount || 0);
            }
        });

        const data = formatResponse(200, false, "Sales data fetched successfully", Object.values(campCountMap));
        res.status(200).json(data);
        return;
    } catch (error: any) {
        const data = formatResponse(500, true, error.message, null);
