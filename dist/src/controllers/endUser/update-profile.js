...
const updateProfile = async (req, res) => {
    try {
        ...
        if (req.body.country_id) {
            const country = await services_1.countriesService.getCountryById(req.body.country_id);
            if (!country) {
                const data = (0, helpers_1.formatResponse)(400, true, "Country not found.", null);
                res.status(400).json(data);
                return;
            }
        }
        if (req.body.national_id_type) {
            const national_id_type = await services_1.nationalTypeService.getNationalTypeById(req.body.national_id_type);
            if (!national_id_type) {
                const data = (0, helpers_1.formatResponse)(400, true, "National id type not found.", null);
                res.status(400).json(data);
                return;
            }
        }
        ...
        if (req.files.national_id && req.files.national_id.length) {
            ...
            if (user.national_id) {
                await (0, helpers_1.deleteFile)(helpers_1.userDir + user.national_id);
            }
        }
        if (req.files.user_image && req.files.user_image.length) {
            user_image = req.files.user_image[0].filename;
            if (user.user_image) {
                await (0, helpers_1.deleteFile)(helpers_1.userDir + user.user_image);
            }
        }
        if (req.files.passport_image && req.files.passport_image.length) {
            ...
            if (user.passport_image) {
                await (0, helpers_1.deleteFile)(helpers_1.userDir + user.passport_image);
            }
        }
        if (national_id) {
        ...
        await services_1.userRegisterService.updateProfile(req.decodedToken.data.id, req.body);
