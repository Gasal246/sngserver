...
const getInternetPackageOrderListForEndUser = async (req, res) => {
    ...
    try {
        ...
        const list = await services_1.orderInternetPackageService.getInternetPackageForUser((0, helpers_1.createObjectId)(userData.id), status, userData.base_camp_client_id);
        if (!list || !list.length) {
